 ...
 * @author an
 */
public class All_Trash_Vehicles extends javax.swing.JFrame {
    ...
    /**
     * Creates new form All_Trash_Vehicles
     *
     * @param welcomeMessage
     */
    public All_Trash_Vehicles(String welcomeMessage) {
        ...
        jTable2.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow >= 0 && SwingUtilities.isLeftMouseButton(e)) {
                    try {
                        List<Vehicles_mod> vehicules = controller.getAllTrashVehicles();
                        ...
                        if (veh != null) {
                            ...
                            int year = veh.getYear();
                            String imagePath = veh.getImage(); // Chemin absolu de l'image
                            Long price = veh.getPrice();
                            Long loan_price = veh.getLoan_price();
                            ...
                            switch (option) {
                                case 0: // "Supprimer"
                                    EraseVehiclesQuestion(veh);
                                    break;
                                case 1: // "À louer" ou "À vendre"
                                    RestoreVehiclesQuestion(veh);
                                    break;
                                case 2: // "Modifier"
                            ...
                            JOptionPane.showMessageDialog(All_Trash_Vehicles.this, "Erreur : le véhicule sélectionné n'a pas pu être trouvé.", "Erreur", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(All_Trash_Vehicles.this, "Erreur lors de la récupération des données : " + ex.getMessage(), "Erreur", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
        });
    ...
    public void populateTable() {
        boolean EmptyTrashVehicles = controller.EmptyTrashVehicles();
        DefaultTableModel model = (DefaultTableModel) jTable2.getModel();
        model.setRowCount(0);

        if (EmptyTrashVehicles) {
            ...
            List<Vehicles_mod> vehiclesList = controller.getAllTrashVehicles();  // Fetch all people from the controller
    ...
    private void EraseVehiclesQuestion(Vehicles_mod veh) {
        // Show a dialog with Yes and No options
        int option = JOptionPane.showOptionDialog(
                null,
                "Voulez-vous réellement supprimez le véhicule définitivement ?",
                "Confirmation",
    ...
    private void RestoreVehiclesQuestion(Vehicles_mod veh) {
        // Show a dialog with Yes and No options
        int option = JOptionPane.showOptionDialog(
                null,
                "Voulez-vous réellement Restaurez le véhicule ?",
                "Confirmation",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.QUESTION_MESSAGE,
                null,
    ...
    private void handleRestoreVehicles(Vehicles_mod veh) {
        boolean success = controller.SureRestoreVehicles(veh);

        if (success) {
            JOptionPane.showMessageDialog(this, "Le véhicule a été réstaurer.", "Information", JOptionPane.INFORMATION_MESSAGE);
            populateTable();
        } else {
            JOptionPane.showMessageDialog(this, "Erreur lors de la réstauration du véhicule.", "Erreur", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void handleEraseVehicles(Vehicles_mod veh) {
        boolean success = controller.SureEraseVehicles(veh);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(All_Trash_Vehicles.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new All_Trash_Vehicles(finalMessage).setVisible(true);
            }
        });
    }
